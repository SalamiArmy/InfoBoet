...
def run(keyConfig, message, totalResults=1):
    ...
    if requestText != '':
        ...
        if has_place_results:
            if has_iss_results:
                return format_iss_message(durationSeconds, requestText, startDateTime)
            else:
                return 'I\'m sorry Dave, I\'m afraid I can\'t find the next ISS sighting for ' +\
                                                      requestText.encode('utf-8') + '.'
        else:
            return 'I\'m sorry Dave, I\'m afraid I can\'t find any places for ' +\
                                                  requestText.encode('utf-8') + '.'
    else:
